...
public class SimpleMazeGenerator extends AMazeGenerator{
    ...
    public static Stack<Position> GetAvailableNeighbors(Maze m, Position p) {
        ...
        if (SimpleMazeGenerator.CheckUpNeighbor(m, p)) {
            Position pos = new Position(p.getRowIndex() - 1, p.getColumnIndex());
            k.push(pos);
        }
        if (SimpleMazeGenerator.CheckDownNeighbor(m, p)) {
            Position pos = new Position(p.getRowIndex() + 1, p.getColumnIndex());
            k.push(pos);
        }
        if (SimpleMazeGenerator.CheckLeftNeighbor(m, p)) {
            Position pos = new Position(p.getRowIndex(), p.getColumnIndex() - 1);
            k.push(pos);
        }
        if (SimpleMazeGenerator.CheckRightNeighbor(m, p)) {
            Position pos = new Position(p.getRowIndex(), p.getColumnIndex() + 1);
            k.push(pos);
        }
        return k;
    ...
    public Maze generate(int rows, int columns) {
        ...
        while(!Pathstk.isEmpty())
        {
            ...
            Stack<Position> Neighbors=SimpleMazeGenerator.GetAvailableNeighbors(m,currCell);
